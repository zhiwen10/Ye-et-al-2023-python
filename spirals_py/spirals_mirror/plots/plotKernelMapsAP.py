"""Translated from spirals_mirror/plots/plotKernelMapsAP.m (Figure 3j)."""
...
def plotKernelMapsAP(data_folder, save_folder):
    """Translated from spirals_mirror/plots/plotKernelMapsAP.m"""
    ...
    h3j = plt.figure(figsize=(6, 4))

    # SSp mask with the 8 example points
    ax = h3j.add_subplot(1, 4, 1)
    ...
    # mean kernel maps on MO
    ax = h3j.add_subplot(1, 4, 2)
    ...
    # axon projection intensity on MO
    ax = h3j.add_subplot(1, 4, 4)
    ax.imshow(template1, cmap="gray", alpha=BW_MO)
    for kkk in range(8):
        _imshow_color_overlay(
    ...
    h3j.savefig(save_folder / "Fig3j_AP_mean_activity_axon.png", bbox_inches="tight")
    h3j.savefig(save_folder / "Fig3j_AP_mean_activity_axon.pdf", bbox_inches="tight")
    return h3j
