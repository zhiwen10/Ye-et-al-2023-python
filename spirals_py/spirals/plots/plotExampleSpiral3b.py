"""Translated from spirals/plots/plotExampleSpiral3b.m (Extended Data Fig.1i-j).

Plots the ZYE_0067 example spirals: a 4 x 10 sequence of dF/F and phase
frames (two full-cortex rows and two zoomed rows with detected spirals
overlaid, FigS1j), and the mean image with 7 region pixels plus raw +
phase-colored band-passed time series at those pixels (FigS1i).
"""
...
def plotExampleSpiral3b(T, data_folder, save_folder):
    """Translated from spirals/plots/plotExampleSpiral3b.m; returns (hs1i, hs1j)."""
    ...
    center = np.array([74, 34])
    r = 13
    th2 = np.linspace(0, 360, 8) + 300
    th2 = th2[:-1]
    ...
    hs1j.savefig(save_folder / f"FigS1j_{fname}_{frameS}.png", dpi=300)
    ...
    hs1i = plt.figure(figsize=(7, 5))
    ax1 = hs1i.add_subplot(1, 2, 1)
    ...
    ax2 = hs1i.add_subplot(1, 2, 2)
    for i in range(7):
        ax2.plot(qt, trace_raw[i] * 100 + 3 * i, "k", linewidth=1)
        ax2.add_collection(
            _phase_colored_segments_pos(qt, trace_filt[i], trace_phase[i], 100, 3 * i, cmap_c06)
        )
    ax2.tick_params(labelsize=8)
    ax2.set_xlim(t1[0], t1[-1])
    ax2.set_yticklabels([])
    for i in range(7):
        ax2.text(t1[0] - 0.5, 3 * i, nameList[i], color=color2[i + 2])
    ax2.set_xlabel("Time (s)", fontsize=9)
    ax2.axvline(qt[first_frame_to_plot1 - 1], ls="--")
    ax2.axvline(qt[last_frame_to_plot1 - 1], ls="--")
    ax2.plot([t1[0] + 0.5, t1[0] + 0.5], [0, 2], "r")
    hs1i.savefig(save_folder / f"FigS1i_{fname}_{frameS}_cetc6_3.png", dpi=300)
    plt.show()
    return hs1i, hs1j
